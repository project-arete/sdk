...
12. **Writing a property your role does not source fails silently.** The orchestrator propagates a value by deriving the *opposite* role from the property's source (ARETE.md §2.2). If the write came from the side that does not source that property, there is no opposite role and propagation stops — with no error, no rejection, and no log line at the application. Verified live four ways: a consumer writing `sOut`, and a provider writing `cState`, both land on the writer's own key and never reach the peer; the same holds for a raw write placed directly into a connection property key, which does not cross either. The value is visible in your own namespace, which makes it look as though it worked. If a peer never sees a value you are certain you wrote, check the property's source before suspecting the broker.
